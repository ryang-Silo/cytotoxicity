...
print(f"current working dir is {curdir}")
...
def train_test(args):
    ...
    outmodel_dir = args.output_folder
    os.makedirs(outmodel_dir, exist_ok=True)
    ...
    for ik in range(fold): #(fold, fold+1):
        ...
        trainval_list = os.path.join(curdir, 'train_test_folds/train_fold_'+'%d' % (index)+'_info.txt')
        test_list = os.path.join(curdir, 'train_test_folds/test_fold_'+'%d' % (index)+'_info.txt') 
...
#                 #optimizer.param_groups[1]['lr'] *= 0.1
            ...
            cyto_train = cytoDataset(cv_info=trainval_list, root_dir=data_dir, transform=transforms.Compose([Normaliztion(), Rescale((384,384)),RandomCrop((256,256)),RandomHorizontalFlip(), ToTensor()]))
            ...
            cyto_test = cytoDataset(cv_info=test_list, root_dir=data_dir, transform=transforms.Compose([Normaliztion(), Rescale((384,384)),RandomCrop((256,256)), ToTensor()]))
            ...
            if top1.avg >= best_top1:
                best_top1 = top1.avg
                run.log('best_top1', np.float(best_top1), description=f'current testing results model for fold {index}', step=epoch)
                ckpt = {
                'fold': index,
                'best_top1': best_top1,
                ...
                joblib.dump(value=model, filename='outputs/'+args.log+'_%d.pt' % (index))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="cyto_5fold_ep30_lr0001_job", help='log and save model name')
    parser.add_argument('--finetune', action='store_true', default=False, help='whether finetune other models')
    parser.add_argument('--data_folder', type=str, dest='data_folder', help='data folder mounting point')
    parser.add_argument('--output_folder', type=str, dest='outputs', help='data folder mounting point')
