...
def generate_separate_compound(data_dir, cp_df):
    samples = []
    labels = []
    ...
    for i, compound_name in enumerate(cp_df[0].tolist()):
        ...
        compound_groups, compound_groups_labels = generate_compound_groups(compoundPath, compound_label)
...
class cytoDataset(Dataset):
    def __init__(self, cv_info, root_dir, transform=None):
        ...
        self.cp_samples_info, self.samples_labels = generate_separate_compound(self.data_path, self.compoundList)
    ...
    def __getitem__(self, idx):
        ...
        # combine images
        sample_compound = get_video_data(one_compound_info)
...
def get_video_data(sample_cp_list):
    ...
    for i, image in enumerate(sample_cp_list):
        ...
        if int(ch) in channel_dict.keys():
            ...
            im_arr = mpimg.imread(image) #np.array(Image.open(os.path.join(sample_cp_list, image)))
...
def generate_compound_groups(compoundPath, cp_label):
    # print(f"preparing {len(os.listdir(compoundPath))} images in compound {compoundPath}")
    image_list = os.listdir(compoundPath)
    min_column, max_column = find_comp_column_range(compoundPath)
...
if __name__ == '__main__':
    root_list = '/Volumes/KINGSTON/Orion/reference_dataset_compounds_v1'
    trainval_list = '/Users/ruijingyang/VSProjects/Orion-Cytox/src/cytotoxicity/data/train_test_folds/train_fold_1_info.txt'
    

    BioVid_train = cytoDataset(trainval_list, root_list, transform=transforms.Compose([Normaliztion(), Rescale((384,384)), RandomCrop((256,256)), RandomHorizontalFlip(), ToTensor()]))
    
    dataloader = DataLoader(BioVid_train, batch_size=1, shuffle=True, num_workers=2)
