...
def split_images_by_compound(args):
    ...
    for plate in plates:
        ...
        for cp_ind, compound in enumerate(compound_list):
            ...
            # calculate existing images in compounds
            # skip those missing compounds
            if not os.path.exists(output_compound):
                print(f"compound {compound} doesn't exist here!")
                continue
            
            count = len([name for name in os.listdir(output_compound) if os.path.isfile(os.path.join(output_compound, name))])
            if count >= 300 and count < 320: # or count == 480:
                print(f"problem compound {compound} missed several images!")
                continue
            ...
            #     compound_num += 1
                ...
                print(f"plate {plate}, compound {compound} doesn't have enough images! It only has {count} images")
